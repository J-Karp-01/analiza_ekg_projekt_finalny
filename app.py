...
# 4. Wyświetlenie w Streamlit
st.dataframe(df_qrs, use_container_width=True)

#%%-----------------------------------SEKCJA 4---------------------------------
st.markdown("## $Analiza sygnału$")
...
# EMD
    ...
    fig = make_subplots(rows=n+1, cols=1, shared_xaxes=True)

    # ===== sygnał =====
fig.add_trace(
    go.Scatter(y=ecg, mode='lines'),
    row=1, col=1
)
...
for i in range(n):
    fig.add_trace(
        go.Scatter(y=imfs[:, i], mode='lines'),
        row=i+2, col=1
    )

    # 🔥 PODPISY IMF
    fig.update_yaxes(title_text=f"IMF-{i+1}", row=i+2, col=1)
...
#-------------synchronizacja w układzie sercowo oddechowym----------------------------------
